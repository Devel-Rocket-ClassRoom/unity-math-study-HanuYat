...
public class Assignment_LissajousCurve : MonoBehaviour
{
    ...
    private Vector3 CalculateLissajousPosition(float time)
    {
        ...
        // TODO: 위 공식을 구현하세요
        float phaseX = amplitudeX * Mathf.Sin(2f * Mathf.PI * frequencyX * time + (this.phaseX * Mathf.Deg2Rad));
        float phaseZ = amplitudeZ * Mathf.Sin(2f * Mathf.PI * frequencyZ * time + (this.phaseZ * Mathf.Deg2Rad));

        return initialPosition + new Vector3(phaseX, 0f, phaseZ);
    }
