...
public class CatmullRomDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        currentSegment = Mathf.FloorToInt(currentT * (waypoints.Length - 1));
        ...
        {
            transform.rotation = Quaternion.LookRotation(currentTangent.normalized, Vector3.up);
        }
    ...
    private Vector3 GetWaypoint(int index)
    {
        // TODO
        int n = waypoints.Length;
        int clampedIndex = Mathf.Clamp(index, 0, n - 1);

        return waypoints[clampedIndex].position;
    }
